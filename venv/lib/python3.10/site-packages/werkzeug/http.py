...
def dump_options_header(header: str | None, options: t.Mapping[str, t.Any]) -> str:
    """Produce a header value and ``key=******** parameters separated by semicolons
    ...
    :param options: Parameters to encode as ``key=******** pairs.
...
def dump_header(iterable: dict[str, t.Any] | t.Iterable[t.Any]) -> str:
    """Produce a header value from a list of items or ``key=******** pairs, separated by
    commas ``,``.
...
def parse_dict_header(value: str) -> dict[str, str | None]:
    ...
    for item in parse_list_header(value):
        ...
        if key[-1] == "*":
            # key*=charset''value becomes key=******** where value is percent encoded
...
def parse_options_header(value: str | None) -> tuple[str, dict[str, str]]:
    """Parse a header that consists of a value with ``key=******** parameters separated
    ...
    # Collect all valid key=******** parts without processing the value.
